...
namespace cvutils
{
	...
	static cv::Mat channelDot(const cv::Mat& m1, const cv::Mat& m2)
	{
		...
		cv::Mat m1m2dot;
        cv::reduce(m1m2, m1m2dot, 1, cv::REDUCE_SUM, CV_32F);
	...
	static cv::Mat channelSum(const cv::Mat& m1)
	{
        ...
        cv::reduce(m, m, 1, cv::REDUCE_SUM, CV_32F);
    ...
    inline double normL1(cv::Vec3f x, cv::Vec3f y) {
        return cv::norm(x - y, cv::NORM_L1);
    }

    //Suspect it to be same as L1 for a scalar
    inline double normL2(cv::Vec3f x, cv::Vec3f y) {
